...
export default function CustomerCard({ intent, data, icon }) {
  function getClassFromIntent(intent) {
    if (intent == 'primary') {
      return 'flex flex-col space-y-10 p-4 rounded bg-blue-200  w-full'
    } else if (intent == 'secondary') {
      return 'flex flex-col space-y-10 p-4 rounded bg-yellow-200  w-full'
    } else if (intent == 'warning') {
      return 'flex flex-col space-y-10 p-4 rounded bg-red-200  w-full'
    } else if (intent == 'danger') {
      return 'flex flex-col space-y-10 p-4 rounded bg-red-400  w-full'
    } else if (intent == 'normal') {
      return 'flex flex-col space-y-10 p-4 rounded bg-green-200  w-full'
    } else {
      return 'flex flex-col space-y-10 p-4 rounded bg-grey-400 shadow-lg ring-1 ring-zinc-200 w-full'
    }
  }
  return (
