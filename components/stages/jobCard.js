            ...
            <div className="w-4/5">
              <DatePicker
                ...
                onChange={(values, dateStrings) =>
                  setEntryDate(moment(dateStrings))
                }
              />
            </div>
          </div>
          <div className="flex items-center justify-between">
            ...
            <div className="w-4/5">
              <Dropdown
                ...
                className="w-1/3"
                onChange={(e, data) =>
                  setDriver(usersList.filter((u) => u.key == data.value)[0])
                }
              />
            </div>
          </div>
          <div className="flex flex-col">
            <div className="flex flex-row">
              ...
              {carPlate && (
                <div className="ml-2 rounded shadow-md">
                  <MTextView content={carPlate.eqDescription} selected />
                </div>
              )}
            </div>
            <div className="mt-2 w-4/5">
              <Dropdown
                ...
                onChange={(e, data) => {
                  console.log(
                    eqList.filter((e) => e.key == data.value)[0]?.mileages
                  )
                  setCarPlate(eqList.filter((e) => e.key == data.value)[0])
                  setCurrentMileages(
                    eqList.filter((e) => e.key == data.value)[0]?.mileages
                  )
                }}
