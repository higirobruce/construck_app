...
export default function Reversals() {
  ...
  function getTransactions() {
    fetch(
      ...
      .then((res) => res.json())
      .then((res) => {
        if (!res.error) setTransactions(res)
      })
  ...
  function reverse(
    ...
    totalRevenue = 0,
    totalExpenditure = 0
    // , siteWork, id, date, revenue, expenditure,
  ) {
    if (siteWork) {
      fetch(
        `${url}/works/swreverse/${id}?date=${dispatchDate}&totalRevenue=${totalRevenue}&totalExpenditure=${totalExpenditure}&duration=${duration}`,
        {
          method: 'PUT',
