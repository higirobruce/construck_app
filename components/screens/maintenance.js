...
const Maintenance = () => {
    ...
    const setJobCardsToUpdate = (data) => {
        ...
        if((role == 'workshopManager' && (data && data.status) && data.status == 'requisition')) {
            showModal()
        } else if(role == 'operatorOfficer' || role == 'teamLeader' || (role == 'workshopSupervisor' && data.jobCard_status == 'opened')) {
    ...
    const handleUpdate = () => {
        const payload = {
            ...
            teamApproval: role === 'teamLeader' ? true : teamApproval,
            supervisorApproval: role === 'workshopSupervisor' ? true : row.supervisorApproval,
            isViewed:
                (role === 'workshopManager' && row.status == 'requisition') ?
                    (isReason) ? 'denied' : 'approved' : isViewed,
            status: role != 'workshopManager' ? page == 1 
            ? 'diagnosis'
            : page == 2 || page == 4
            ? 'requisition'
            : page == 5
            ...
            reason: (role == 'workshopManager' && !checkReason) ? '' : reason
        }
        ...
        .then(result => {
            populateJobCards();
            if((page == 2 && result.status == 'requisition' && result.sourceItem == 'Inventory') && role == 'recordOfficer') {
                ...
                setViewPort('list')
            } else if ((page == 2 && result.status == 'requisition' && result.sourceItem == 'Transfer') && role == 'recordOfficer') {
                setPage(5);
            } 
            ...
            else if(role == 'workshopSupervisor' && (result.supervisorApproval == true && result.jobCard_status == 'closed')) {
                populateJobCards();
                setPage(7)
            }
            else if(role == 'recordOfficer' && result.status == 'testing') {
                populateJobCards();
                setViewPort('list')
            }
            else
        ...
        <div className="my-5 flex flex-col space-y-5 px-10">
            ...
            {(viewPort === 'new') && (
                <div className='mt-5 w-1/2'>
                    {role == 'recordOfficer' ? componentList[page] : <h5 className='text-lg font-medium mt-5 text-center'>Logged in user is not authorized for job card creation</h5>}
                    <div className='flex mt-10 space-x-5'>
                        {page != 0 && 
                            <MSubmitButton submit={() => setPage(page - 1)} label={`Go to Previous`} intent={'primary'} />
                        }
                        {role == 'recordOfficer' && <MSubmitButton submit={page == 0 ? handleSubmit : handleUpdate} label={`${page == 2 ? `Submit Request` : page == 0 ? `Create Job Card` : `Save & Continue`}`} />}
                    </div>
                </div>
            )}
            ...
            {viewPort == 'operatorView' && (
                <div className='mt-5 w-3/4'>
                    ...
                    />
                    <div className='flex mt-10 space-x-5'>
                        {role != 'teamLeader' && <MSubmitButton intent='danger' submit={() => {
                                setTeamApproval(false);
                                setViewPort('list')
                            }} 
                            intentColor={'danger'}
                        ...
                        <MSubmitButton 
                            ...
                            intent={'success'}
                            intentColor={'success'}
                            label={`${role == 'workshopSupervisor' ? 'Authorise Gate Pass' : role == 'teamLeader' ? 'Validate Repairs' : 'Save & Continue'}`} 
                        />
                    </div>
                </div>
            )}
